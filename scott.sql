...
SELECT DISTINCT jop, deptno FROM emp;

-- 별칭
SELECT e.ENAME , e.SAL ,sal * 12 + comm annsal, comm FROM EMP e ;

SELECT e.ENAME , e.SAL ,sal * 12 + comm AS "annsal", comm FROM EMP e ;

SELECT e.ENAME , e.SAL ,sal * 12 + comm AS annsal, comm FROM EMP e ;

SELECT e.ENAME , e.SAL ,sal * 12 + comm AS 연봉, comm FROM EMP e ;

-- 별칭에 공백이 들어가는 경우 ""로 반드시 묶어야 함
SELECT ename 사원명, sal 급여, sal * 12 + comm AS "연 봉", comm 수당 FROM EMP e ;

-- 원하는 순서대로 출력 데이터를 정렬
-- 오름차순일 때 asc 생략 가능(디폴트가 asc)
...
-- 8) IS NULL : 널과 같다
-- 9) 집합 연산자 : UNION(합집합), MINUS(차집합), INTERSECT(교집합)
...
-- 문자 대소비교
-- 사원명의 첫 문자가 F와 같거나 F 보다 뒤에 있는 사원을 조회
SELECT *
FROM EMP e 
WHERE e.ENAME  >= 'F';
...
UNION 
SELECT E.EMPNO, E.JOB , E.SAL, E.DEPTNO  FROM EMP e  WHERE E.DEPTNO = 20;
...
-- 내장함수
-- 1) 문자함수
-- 대소문자를 바꿔주는 함수 : uppr(), lower(), initcap()
...
-- 두 문자열 데이터를 합치기 : CONCAT(문자열1, 문자열2), 문자열1 | 문자열2 | 문자열3
-- 특정 문자 제거 : TRIM(), LTRIM(), RTRIM()

-- 사원이름 대문자, 소문자, 첫만자만 대문자로 변경
...
-- INSTR(대상문자열, 위치를 찾으려는 문자, 시작위치, 시작위치에서 찾으려는 문자가 몇 변째인)
SELECT
...
SELECT
	...
	to_char(sysdate, 'HH24:MI:SS'), -- 17:58:55
	to_char(sysdate, 'HH12:MI:SS AM'), -- 05:58:55 오후
	to_char(sysdate, 'HH:MI:SS PM') -- 05:58:55 오후
FROM
	dual;
